...
namespace CtYun
{
    internal class CtYunApi
    {
        ...
        /// <summary>远程验证码识别兜底接口（本地模型不可用时使用），启动时可由配置覆盖。</summary>
        internal static string RemoteOcrUrl { get; set; } = "https://orc.1999111.xyz/ocr";
        ...
        private async Task<byte[]> GetLoginCaptcha(string userphone)
        {
            try
            {
                return await client.GetByteArrayAsync("https://desk.ctyun.cn:8810/api/auth/client/captcha?height=36&width=85&userInfo=" + userphone + "&mode=auto&_t=" + DateTimeOffset.Now.ToUnixTimeMilliseconds());
            }
            catch (Exception ex)
            {
        ...
        /// <summary>供自检（selftest）使用：拉取一张登录验证码图片，无需登录态。</summary>
        internal Task<byte[]> DownloadLoginCaptchaForSelfTest(string userphone) => GetLoginCaptcha(userphone);

        private async Task<byte[]> GetSmsCodeCaptcha()
        {
            try
            {
                return await GetByteAsync("https://desk.ctyun.cn:8810/api/auth/client/validateCode/captcha?width=120&height=40&_t=" + DateTimeOffset.Now.ToUnixTimeMilliseconds());
            }
            catch (Exception ex)
            {
        ...
        private Task<string> GetCaptcha(byte[] img)
        {
            if (img == null || img.Length == 0)
            {
                return Task.FromResult("");
            }
            return OcrService.RecognizeAsync(img, RemoteOcrUrl);
        }

        public async Task<List<Desktop>> GetLlientListAsync()
        {
            var result = await PostAsync("https://desk.ctyun.cn:8810/api/desktop/client/pageDesktop", new StringContent("{\"getCnt\":20,\"desktopTypes\":[\"1\",\"2001\",\"2002\",\"2003\"],\"sortType\":\"createTimeV1\"}", Encoding.UTF8, "application/json"), AppJsonSerializerContext.Default.ResultBaseClientInfo);
            if (result.Success && result.Data?.DesktopList != null)
            {
                return result.Data.DesktopList;
            }
            Utility.WriteLine(ConsoleColor.Red, $"获取云电脑列表失败：{result.Msg}");
            return null;
        }
